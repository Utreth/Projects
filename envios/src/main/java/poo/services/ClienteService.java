...
public class ClienteService implements Service<Cliente> {
    ...
    public JSONObject getAll() {
        try {
            ...
            return new JSONObject().put("message", "ok").put("data", data).put("length", data.length());
    ...
    public Cliente getUpdated(JSONObject newData, Cliente current) {
        ...
        if (newData.has("nombre")) {
            ...
            update.put("nombre", Utils.stringOk("nombre", 10, newData));
        }
    ...
    public JSONObject size() {
        ...
        return new JSONObject().put("message", "ok").put("size", list.size());    }
